...
The F-statistic is 11.91 with a p-value equal to 0.0003. We clearly reject the null hypothesis of equal means for all three drug groups. 
...
Exercise 1
We will use the anova() function to perform ANOVA test in R. This function accepts a model
object as input which corresponds a set of values based on a factor column as our treatment
variable. Generally, if the measurements and treatments are in a dataframe called ‘our_data’ 
we will use lm function to create this model as:
...
2 – Investigate if different methods of ventilation during anesthesia has any effect on the red folates. 
...
2-	Investigate if Tanner levels have any significant effect on amount of insulin-like growth 
factor igf1.
...
and shortly after administration of enalaprilat. Column hr is the patient heart rate, column 
subject has the subject ids and column time indicates time between administration of enalaprilat 
and measuring the heart rate. By performing a two-way ANOVA, investigate if time (or subject) 
has any effect on the heart rates. 
